...
- 对**契约声明集内**（`combos/<combo>/copier.yml`）的参数：底座 enabled choices vs 契约模板 `{% if x == 'v' %}` 显式覆盖（启发式扫描 + `{% else %}` 兜底判断）
- **硬缺口（阻止）**：启用取值未显式覆盖且无 else 兜底（渲染为空）；或契约未条件引用该参数
- **提示（不阻止）**：启用取值经 else 兜底——如 `auth_mode` 新增 `jwt` 落入「无认证」else 分支，提示确认语义（`jwt` 的感知同时由检查 1 漂移「启用取值变化」送达）
- 声明集外的底座参数（`python_version` / `db_dialect` 等）契约本就不覆盖，不判缺口
- **数据基准**：检查 2 以底座**工作树** `params.json` 为准——check-drift 信号版本与 HEAD 可能不一致（信号发出后 HEAD 又推进），用最新 HEAD 判断，方向偏保守（更多提示），可接受
...
| 前端 vite proxy | **已解决**：契约 §4.1 注明「启用 proxy（:8000，后端默认 8000）」；底座模板 vite.config.ts 注释示例端口已随契约修正为 8000（并注明以 CONTRACT.md §4.1 为准） | — |
